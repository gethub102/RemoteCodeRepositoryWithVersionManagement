...
std::string Socket::getrecvLineString() {
	std::string line;
	char ch;
...
// get the body buffer from string msg
bool Socket::getBodyBuffer(size_t bytes, byte * buffer)
{
...
/* get the buffer body */
std::string Socket::getBody(size_t length)
{
	auto bodystring = _msgString.substr(_pos, _pos + length);
	return bodystring;
}
...
*/
bool Socket::send(size_t bytes, byte* buffer) //break down and send
{
...
bool Socket::sendString(const std::string& str, byte terminator)
{
	...
	const byte* pBuf = &(*str.begin()); //convert string to bytes and send
...
bool SocketConnecter::connect(const std::string& ip, size_t port) {
	...
	for (ptr = result; ptr != NULL; ptr = ptr->ai_next) { // Attempt to connect to an address until one succeeds
		...
		iResult = ::connect(socket_, ptr->ai_addr, (int)ptr->ai_addrlen); //conect the socket
...
void ClientHandler::operator()(Socket& socket_)
{
	while (true)
	{
		...
		if (command.size() == 0 || command == "Done")
		{
...
void clientTestBufferHandling(Socket& si)
{
	...
	bool ok;
	testHandleBuffer(ok, BufLen, buffer, si);
}
/* test client handler buffer */
void testHandleBuffer(bool ok, const int BufLen, Socket::byte* buffer, Socket& si) {
	while (true)
	{
