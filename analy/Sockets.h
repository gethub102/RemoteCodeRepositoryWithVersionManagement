...
*  Package Operations:
*  -------------------
*  every file should have meta data. EXE -> Task
										 -> Test Analysis  ==> create Meta Data xml(contains dependency relationship);

	repository will generate meta data.
	HTTP style message: new line + (first)command + ToAddr(IP Port:listening port) + FromAddr + comtentslength + emptyline
	+ contents(messages).
	sever will have one recive handler(socket/stream + thread) + shared queue for every sender(), and then enQ
...
public:
	...
	std::string getrecvLineString();
	bool getBodyBuffer(size_t bytes, byte * buffer);
	void setmsgString(std::string& msg) { _msgString = msg; };
	std::string getBody(size_t length);
protected:
...
bool SocketListener::start(CallObj& co)
{
		...
		[&]()
	{
		...
		while (!acceptFailed_)
		{
			...
			Socket clientSocket = accept();    // uses move ctor !!! create new socket in cloud!!!!!
...
template<typename CallObj>
bool SocketListener::clientStart(CallObj& co)
{
	if (!bind())
	{
		return false;
	...
	{
		//Verbose::show("server waiting for connection");
		StaticLogger<1>::write("\n  client waiting for connection and response");
		...
		while (!acceptFailed_)
		{
			...
			Socket serverSocket = accept();    // uses move ctor
			if (!serverSocket.validState()) {
				continue;
			}
			//Verbose::show("server accepted connection");
			StaticLogger<1>::write("\n  server accepted connection");
			...
			std::thread clientThread(std::ref(co), std::move(serverSocket));
			clientThread.detach();  // detach - listener won't access thread again
		}
		//Verbose::show("Listen thread stopping");
		StaticLogger<1>::write("\n  Listen thread stopping");
