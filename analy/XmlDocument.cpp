...
std::vector<std::string> XmlDocument::getElementDescendents(XmlDocument& doc, std::string testTag)
{
	std::vector<std::string> valueOfTag;
	//std::string testTag = "child1";
	title("Client testing doc.element(" + testTag + ").descendents()");
	...
	if (found.size() > 0)
	{
		for (auto pElem : found)
		{
			if (pElem->tag() != "")
				...
				valueOfTag.push_back(pElem->value());
			}
		}
	}
	else
...
int main()
{
  ...
  XmlDocument::getElementDescendents(doc, "child1");
